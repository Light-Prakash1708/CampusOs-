...
/**
 * Raises a readdressal case. Posts to the shared `/api/grievances` route, which
...
export function NewCaseForm({
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      router.push(id ? `/student/readdressal/${id}` : '/student/readdressal');
      router.refresh();
    } catch {
      setError({
      ...
      <Card>
        <CardBody>
          ...
          <p className="mt-1 text-[13px] leading-relaxed text-muted">
            Your institution has not enabled any readdressal category for student accounts, so a
    ...
    <form onSubmit={submit} className="space-y-5" noValidate>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button asChild variant="ghost">
          <Link href="/student/readdressal">Cancel</Link>
        </Button>
      </div>
    </form>
  );
